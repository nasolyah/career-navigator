...
// Цены за миллион токенов, тарифы Google на сентябрь 2026
const PRICES = {
...
export async function generateMap(payload, apiKey, opts = {}) {
  ...
  try {
    ...
    for (const model of models) {
      ...
      map.model = model;
      // Реальный расход по данным самого Google — чтобы стоимость можно было проверить, а не оценивать
      ...
      map.usage = {
        inputTokens: inTok,
        outputTokens: outTok,
        totalTokens: u.totalTokenCount || inTok + outTok,
        costUsd: Number((inTok / 1e6 * price.in + outTok / 1e6 * price.out).toFixed(6)),
      };
      return map;
    }
