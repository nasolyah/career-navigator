...
import { CATALOG, CATALOG_BY_ID, SCHOOL_SUBJECTS } from './catalog.mjs';
...
export const MODELS = ['gemini-3.6-flash', 'gemini-3-flash', 'gemini-2.5-flash', 'gemini-2.0-flash'];
...
Правила:
1. Линия — это пересечение двух (иногда трёх) интересов ученика, а не один «главный» интерес. Каждая линия — своя комбинация. Поле interests линии — id из списка интересов, который ты сам сформируешь из ответов.
2. Интересы (interests): 3–5 станций-стартов. title — 1–2 слова, id — латиницей, desc — одна фраза о том, как это проявляется у ученика (из его ответов). Ответ «кем видят родители» — НЕ интерес ученика: линию на нём не строй, можно упомянуть в why как контраст «своё vs навязанное».
3. У каждой линии ровно 4 станции — навыки или чекпоинты по пути к специальности, от простого к сложному. status: done — уже есть (судя по ответам и оценкам), partial — наполовину, gap — ещё нет. Будь честным: у 9-классника большинство станций gap, done — только то, что реально следует из ответов.
4. Пересадки: если один навык нужен двум линиям (например, «Python-основы» для геймдизайна и биоинформатики), поставь у обеих станций одинаковый sharedId и одинаковый title. Не больше трёх пересадок на карту, пересадка соединяет ровно две линии.
...
7. catalogId — id профессии из справочника. Специальность и вузы берутся из справочника по этому id, ты их не пишешь. Если точного совпадения нет — выбери ближайшую профессию, не выдумывай.
...
10. Язык — русский, тон — спокойный, честный, на равных. Верни только JSON по схеме.`;
...
export function buildUserPrompt(payload) {
  ...
  return `Ученик: ${p.name || 'без имени'}, ${p.grade || 9} класс. Оценки за четверть: ${subjects}.
...
export async function generateMap(payload, apiKey, opts = {}) {
  ...
  try {
    ...
    for (const model of models) {
      ...
      if (res.status === 404) { lastErr = new Error(`${model}: недоступна`); continue; } // модель не выдана ключу — пробуем следующую
...
export function normalizeMap(raw) {
  ...
  let lines = (raw.lines || []).slice(0, 5).map((ln, i) => {
    ...
    return {
      ...
      universities: cat ? cat.universities.slice() : [],
      stations,
